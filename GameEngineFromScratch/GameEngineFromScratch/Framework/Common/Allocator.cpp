...
WS::Allocator::Allocator(size_t data_size, size_t page_size, size_t alignment)
    : m_pPageList(nullptr), m_pFreeList(nullptr)
{
...
void WS::Allocator::Reset(size_t data_size, size_t page_size, size_t alignment)
{
    FreeAll();
...
#if defined(_DEBUG)
void WS::Allocator::FillFreePage(PageHeader* pPage)
{
    // page header
    pPage->pNext = nullptr;
...
void WS::Allocator::FillFreeBlock(BlockHeader* pBlock)
{
    // block header + data
...
void WS::Allocator::FillAllocatedBlock(BlockHeader* pBlock)
{
    // block header + data
...
BlockHeader* WS::Allocator::NextBlock(BlockHeader* pBlock)
{
